...
def preprocess_gradient(g: Tensor, p: float = 10.0) -> Tuple[Tensor, Tensor]:
    ...
    threshold = torch.exp(torch.tensor(-p, device=g.device, dtype=g.dtype))
    ...
    sign_g = torch.where(
        ...
        torch.exp(torch.tensor(p, device=g.device, dtype=g.dtype)) * g  # Linear scaling for small values
    )
    return log_g, sign_g
            ...
            if should_train_mlp:
                ...
                if training_samples:
                    ...
                    if surrogate_losses:
                        ...
                        # Store for logging
                        self._last_surrogate_loss = total_surrogate_loss.item()
