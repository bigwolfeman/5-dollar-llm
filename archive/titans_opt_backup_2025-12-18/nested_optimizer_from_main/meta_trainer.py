...
class SimplifiedMetaTrainer:
    ...
    def compute_proxy_loss(
        ...
        """
        Compute proxy meta-loss using direct loss-delta signal (paper-aligned).

        Instead of correlation-based heuristics, directly uses the loss
        delta as the training signal:
        - If loss decreased: reward by pulling multipliers toward current
        - If loss increased: penalize by pushing away from current

        This approximates the true meta-gradient without full unrolling.
        """
        if len(self.loss_history) < 10:
            # Not enough history - L2 regularization toward 1.0
            return ((current_multipliers - 1.0) ** 2).mean() * 0.01

        # Compute loss delta (positive = improvement)
        loss_delta = self.loss_history[-2] - current_loss  # Previous - current

        # Normalize by loss scale (avoid huge gradients)
        normalized_delta = loss_delta / (abs(current_loss) + 1e-8)
        normalized_delta = max(-1.0, min(1.0, normalized_delta))  # Clamp

        # Loss function based on improvement signal
        if normalized_delta > 0:
            # Loss is improving - reward current behavior (small loss to maintain)
            target = current_multipliers.detach()
            loss = 0.01 * ((current_multipliers - target) ** 2).mean()
        else:
            # Loss is worsening - penalty proportional to how bad
            # Push toward default (1.0) with strength proportional to worsening
            penalty_strength = abs(normalized_delta)
            loss = penalty_strength * ((current_multipliers - 1.0) ** 2).mean()

        # Add exploration bonus when stuck (loss hasn't improved in 20+ steps)
        if len(self.loss_history) >= 20:
            long_term_improvement = self.loss_history[0] - self.loss_history[-1]
            if long_term_improvement < self.improvement_threshold:
                # Stuck - encourage trying different values
                diversity_loss = -0.05 * (current_multipliers - 1.0).abs().mean()
